...
import com.tpb.coinz.nonNullAny
...
class LocationListeningEngineTest {
    ...
    companion object {
        ...
        fun beforeClass() {
            ...
            `when`(mockLocationProvider.addListener(com.tpb.coinz.nonNullAny())).then {
                listeners.add(it.getArgument(0))
            }
            `when`(mockLocationProvider.removeListener(nonNullAny())).then {
